---
title: "Guía de Extensión y Creación de Plugins: daedalus"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de daedalus"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-daedalus-plugins)=
# Guía de Extensión y Plugins: daedalus

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`daedalus`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-daedalus-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`daedalus` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para daedalus`.
- **Punto de Entrada Oficial**: `daedalus.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-daedalus-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para daedalus."""
    name = "custom_rule"
...
Para que `daedalus` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."daedalus.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `daedalus` la reconozca:
...
daedalus plugins list
````

---

(manual-daedalus-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `daedalus` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = daedalus.ejecutar_analisis(
...
(manual-daedalus-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `daedalus` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar daedalus
        run: pip install -e ./daedalus
        
      - name: Ejecutar Auditoría
        run: daedalus check src/ include/ --json > reporte.json
````

---

(manual-daedalus-plugins-ejercicios)=
